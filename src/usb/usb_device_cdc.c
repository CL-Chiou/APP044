...
#define DRIVER_DATA_ADDRESS_TAG
    ...
    !defined(CONTROL_BUFFER_ADDRESS_TAG) || !defined(DRIVER_DATA_ADDRESS_TAG)
#error \
    "One of the fixed memory address definitions is not defined.  Please define the required address tags for the required buffers."
#endif
...
void USBCheckCDCRequest(void) {
     ...
     */
    if ((SetupPkt.bIntfID != CDC_COMM_INTF_ID) && (SetupPkt.bIntfID != CDC_DATA_INTF_ID)) return;
    ...
    switch (SetupPkt.bRequest) {
        ...
        case SEND_BREAK:  // Optional
            inPipes[0].info.bits.busy = 1;
            if (SetupPkt.wValue ==
                0xFFFF)  // 0xFFFF means send break indefinitely until a new SEND_BREAK command is received
            {
...
void CDCInitEP(void) {
    ...
    OldSerialStateBitmap.byte =
        !SerialStateBitmap.byte;  // To force firmware to send an initial serial state packet to the host.
...
void CDCNotificationHandler(void) {
    ...
    if (UART_DTS == USB_CDC_DSR_ACTIVE_LEVEL)  // UART_DTS must be defined to be an I/O pin in the hardware profile to
                                               // use the DTS feature (ex: "PORTXbits.RXY")
    {
    ...
    if ((SerialStateBitmap.byte != OldSerialStateBitmap.byte) && (!USBHandleBusy(CDCNotificationInHandle))) {
        ...
        CDCNotificationInHandle = USBTransferOnePacket(CDC_COMM_EP, IN_TO_HOST, (uint8_t *)&SerialStatePacket,
                                                       sizeof(SERIAL_STATE_NOTIFICATION));
...
uint8_t getsUSBUSART(uint8_t *buffer, uint8_t len) {
    ...
    if (!USBHandleBusy(CDCDataOutHandle)) {
         ...
         */
        for (cdc_rx_len = 0; cdc_rx_len < len; cdc_rx_len++) buffer[cdc_rx_len] = cdc_data_rx[cdc_rx_len];
