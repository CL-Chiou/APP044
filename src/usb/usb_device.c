...
void USBDeviceInit(void) {
    ...
    USB1msTickCount = 0;  // Keeps track of total number of milliseconds since calling USBDeviceInit() when first
                          // initializing the USB module/stack code.
...
void USBCancelIO(uint8_t endpoint) {
    if (USBPacketDisable == 1) {
        ...
        pBDTEntryIn[endpoint]->Val &= _DTSMASK;  // Makes UOWN = 0 (_UCPU mode).  Deactivates endpoint.  Only sends
                                                 // NAKs.
        pBDTEntryIn[endpoint]->Val ^=
            _DTSMASK;  // Toggle the DTS bit.  This packet didn't get sent yet, and the next call to
                       // USBTransferOnePacket() will re-toggle the DTS bit back to the original (correct) value.
...
static void USBCtrlEPServiceComplete(void) {
    ...
    } else  // A module has claimed ownership of the control transfer session.
    {
        if (SetupPkt.DataDir == USB_SETUP_DEVICE_TO_HOST_BITFIELD) {
            ...
            //        host performs early termination.  If enabled, the USB_ENABLE_STATUS_STAGE_TIMEOUTS
            //        usb_device_config.h option can take care of this for you.
...
// control transfer, so we should re-initialize our timeout counter.
    ...
    // Check if the last transaction was on EP0 OUT endpoint (of any kind, to either the even or odd buffer if ping pong
    // buffers used)
...
static void USBCtrlTrfOutHandler(void) {
    if (controlTransferState == CTRL_TRF_RX) {
        USBCtrlTrfRxService();  // Copies the newly received data into the appropriate buffer and configures EP0 OUT for
                                // next transaction.
...
static void USBCtrlTrfInHandler(void) {
    ...
    } else  // must have been a CTRL_TRF_RX status stage IN packet (<setup><out><out>...<IN>  <-- this last IN just
            // occurred as the status stage)
    {
...
static void USBStdFeatureReqHandler(void) {
    ...
    if ((SetupPkt.bFeature == USB_FEATURE_ENDPOINT_HALT) &&
        (SetupPkt.Recipient == USB_SETUP_RECIPIENT_ENDPOINT_BITFIELD) && (SetupPkt.EPNum != 0) &&
        (SetupPkt.EPNum <= USB_MAX_EP_NUMBER) && (USBDeviceState == CONFIGURED_STATE)) {
...
#endif  // end of #if (USB_PING_PONG_MODE == USB_PING_PONG__ALL_BUT_EP0) || (USB_PING_PONG_MODE ==
...
#if (USB_SPEED_OPTION == USB_LOW_SPEED)
#warning \
    "For low speed USB applications, read the function comments for the USBIncrement1msInternalTimers() function, and implement code to call this function periodically."
#endif
