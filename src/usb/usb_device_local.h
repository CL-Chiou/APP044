...
#warning \
    "Disabling the suspend handler is not recommended.  Proper suspend handling is required to create a compliant USB device."
#else
...
#warning \
    "Disabling the wake from suspend handler is not recommended.  Proper suspend handling is required to create a compliant USB device."
#else
#define USB_WAKEUP_FROM_SUSPEND_HANDLER(event, pointer, size) \
    USER_USB_CALLBACK_EVENT_HANDLER((USB_EVENT)event, pointer, size)
#endif
