...
void ADC1Initialize(void) { /*TAD >= 117.6nS*/
    ...
    AD1CON1bits.ADDMABM =
        1;  // 1 = DMA buffers are written in the order of conversion. The module provides an address to the DMA
    ...
    AD1CON2bits.BUFS  = 0;
    AD1CON2bits.SMPI  = 0b00011;  // 00011 = Generates interrupt after completion of every 4th sample/conversion
                                  // operation
    ...
    AD1CON4bits.ADDMAEN =
        0;  // 0 = Conversion results are stored in ADCxBUF0 through ADCxBUFF registers; DMA will not be used
#else
    AD1CON4bits.ADDMAEN = 1;  // 1 = Conversion results are stored in ADCxBUF0 register for transferring to RAM using
                              // DMA
#endif
