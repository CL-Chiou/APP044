...
/*Beep*/
const uint8_t BzMusic[29] = {100, 100, 0, 0,   100, 100, 0, 0,   100, 0, 100, 0,   100, 100, 0,
                             0,   100, 0, 100, 0,   100, 0, 100, 100, 0, 0,   100, 0,   100};
    ...
    switch (++Task) {
        ...
        case 0:
            if (SwitchStatus.byteTACT != 0 || SwitchStatus.byteDIP != 0) { /*Freq: 50Hz*/
                LEDTurnOn(LED_D4);
                if ((PreviousSwitchStatus.byteTACT != SwitchStatus.byteTACT) ||
                    (PreviousSwitchStatus.byteDIP != SwitchStatus.byteDIP))
                    flagCAN_Tx1shot = 1;
                if (MultiTaskCnt_1000ms <= 50 && SwitchStatus.byteTACT != 0)
                    ++MultiTaskCnt_1000ms;
                else if (SwitchStatus.byteTACT == 0x00 && SwitchStatus.byteDIP == 0x00) {
                    MultiTaskCnt_1000ms  = 0;
                    flagCAN_TxContinuous = 0;
                } else if (MultiTaskCnt_1000ms > 50)
                    flagCAN_TxContinuous = 1;
                // ID = 0x18ABCDEF
                if ((C1TR23CONbits.TXREQ3 == 0 && flagCAN_Tx1shot == 1) || flagCAN_TxContinuous) {
                    ...
                    Ecan1WriteTxMsgBufData(
                        3, 8, (uint16_t)ECAN1MessageBuffers.wordData0, (uint16_t)ECAN1MessageBuffers.wordData1,
                        (uint16_t)ECAN1MessageBuffers.wordData2, (uint16_t)ECAN1MessageBuffers.wordData3);
        ...
        case 1:
            if (SwitchStatus.TACT._BT3 == 1) {
                ...
                flagMusic1shot = 0;
            if (POT_VAULE >= 3150 || SwitchStatus.TACT._BT5 != 0 ||
                (CANIdentifier == (CANRX_ID_1 + 1) && SwitchStatus.DIP._DSW6 == 1) || flagBzMode)
                BzOutput = 1;
            else
                BzOutput = 0;
            Nop();
        ...
        case 6:
            if (++MultiTaskCnt_100ms >= 5) { /*Freq: 10Hz*/
                ...
                if (C1TR01CONbits.TXREQ1 == 0) {
                    ...
                    Ecan1WriteTxMsgBufData(1, 2, (uint16_t)POT_VAULE, (uint16_t)0x00, (uint16_t)0x00, (uint16_t)0x00);
        ...
        case 11:
            if (++MultiTaskCnt_500ms >= 25) { /*Freq: 2Hz*/
                ...
                if (C1TR23CONbits.TXREQ2 == 0) {
                    ...
                    Ecan1WriteTxMsgBufData(2, 8, (uint16_t)ADC1CH0123_mV[0], (uint16_t)ADC1CH0123_mV[1],
                                           (uint16_t)ADC1CH0123_mV[2], (uint16_t)ADC1CH0123_mV[3]);
    ...
    if (Task % 5 == 0) {
        if (++MultiTaskCnt_10ms >= 2) {
            ...
            if (C1TR01CONbits.TXREQ0 == 0) {
                ...
                Ecan1WriteTxMsgBufData(0, 1, (uint16_t)SystemClock.Second, (uint16_t)0x00, (uint16_t)0x00,
                                       (uint16_t)0x00);
...
void UpdateClock(void) {
    ...
    if (SystemClock.Second >= 60) {
        ...
        if (SwitchStatus.byteDIP == 0x00) {
            if ((SystemClock.Hour == 17 && SystemClock.Minute == 0) ||
                (SystemClock.Hour == 11 && SystemClock.Minute == 25))
                BzMode._2 = 1;
            else if (SystemClock.Hour == 11 && SystemClock.Minute == 0)
                BzMode._1 = 1;
        }
        if (SystemClock.Minute >= 60) {
            ...
            if (SystemClock.Hour >= 24) SystemClock.Hour = 0;
        }
    }
}
...
void Debounce(uint16_t Input, eDebounceSwitch_t Switch) {
    ...
    if (Input) {
        ...
        if (SwitchOnCnt[Switch] < SwitchDebounceTime)
            ++SwitchOnCnt[Switch];
        else
            *Output |= BitOn;
    } else {
        SwitchOnCnt[Switch] = 0x00;
        if (SwitchOffCnt[Switch] < SwitchDebounceTime)
            ++SwitchOffCnt[Switch];
...
void CheckBzMode(void) {
    ...
    if ((BzMode0Duty < 50 && BzMode._0) || (BzMode1Duty < 150 && BzMode._1) ||
...
void __attribute__((interrupt, no_auto_psv)) _C1Interrupt(void) {
    ...
    if (C1INTFbits.RBIF) {
        ...
        CANIdentifier = ECAN1MessageBuffers.IDE_BIT
                            ? ((uint32_t)ECAN1MessageBuffers.SID << 18) + ECAN1MessageBuffers.EID
                            : ((uint32_t)ECAN1MessageBuffers.SID);
        Nop();
    }
}
