...
__eds__ extern ECAN1MessageBuffer_t ecan1msgBuf __attribute__((space(eds), aligned(ECAN1_MSG_BUF_LENGTH * 16)));
...
#define mECAN_DMA2MsgBufWord(ECANMessageBufferWord, buf)                                              \
    {                                                                                                 \
        ...
        ECANMessageBufferWord.EID =                                                                   \
            ((uint32_t)(ecan1msgBuf[buf][1] & 0x0fff) << 6) + ((ecan1msgBuf[buf][2] & 0xfc00) >> 10); \
