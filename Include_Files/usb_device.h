...
typedef enum {
    ...
    EVENT_CONFIGURED
        /*DOM-IGNORE-BEGIN*/
        = EVENT_DEVICE_STACK_BASE /*DOM-IGNORE-END*/,
...
/*DOM-IGNORE-BEGIN*/
#define USBDeferOUTDataStage()              \
    {                                       \
        USBDeferOUTDataStagePackets = true; \
...
/*DOM-IGNORE-BEGIN*/
#define USBDeferStatusStage() \
    { USBDeferStatusStagePacket = true; }
/*DOM-IGNORE-END*/
...
/*DOM-IGNORE-BEGIN*/
#define USBDeferINDataStage()              \
    {                                      \
        USBDeferINDataStagePackets = true; \
...
USB_HANDLE USBGetNextHandle(uint8_t ep_num, uint8_t ep_dir);
/*DOM-IGNORE-BEGIN*/
#define USBGetNextHandle(ep_num, ep_dir) \
    ((ep_dir == OUT_FROM_HOST) ? ((USB_HANDLE)pBDTEntryOut[ep_num]) : ((USB_HANDLE)pBDTEntryIn[ep_num]))
/*DOM-IGNORE-END*/
...
void USBEP0Receive(uint8_t *dest, uint16_t size, void(*function));
/*DOM-IGNORE-BEGIN*/
#define USBEP0Receive(dest, size, function)    \
    {                                          \
        outPipes[0].pDst.bRam      = dest;     \
        outPipes[0].wCount.Val     = size;     \
        outPipes[0].pFunc          = function; \
        outPipes[0].info.bits.busy = 1;        \
    }
/*DOM-IGNORE-END*/
